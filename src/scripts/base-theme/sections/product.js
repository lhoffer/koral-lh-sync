  ...
  new Vue({
    ...
    mounted() {
      ...
      if(groupByType == 'tag' && groupByPrefix && prod) {
        ...
        url = `/collections/all/${encodeURIComponent(prod.product_type)}?view=json`
      }
      if(url) {
        $.get(url).then(raw => {
          try{
    ...
    methods: {
      ...
      addToCart() {
        ...
        const data = {id: this.variant.id, quantity: this.quantity};
        if(this.giftMessage) {
          data.properties = {"Gift Message": this.giftMessage};
        }
        this.addItem(data)
          .then(_ => {
            this.openCartDrawer();
