<template>
  ...
  <div class="collection__grid" v-else-if="list && list.length">
    <product v-for="group in getList" :key="group.key" :quickshop-type="'inline'" :product-group="group" :collection-handle="collectionHandle"></product>
  </div>
</template>

<script>
import { mapState } from 'vuex'
import Product from '~/base-theme/components/product.vue'
